...
export default function SettingsManagement() {
  ...
  const [robuxRate, setRobuxRate] = useState('140');
    ...
    <div>
      ...
      <div className="glass-panel" style={{ padding: '2rem', maxWidth: '800px' }}>
        <div style={{ display: 'grid', gap: '1.5rem' }}>
          ...
          <div className={styles.formGroup}>
            <label style={{ color: 'var(--color-text-muted)', marginBottom: '0.5rem', display: 'block' }}>Tỉ giá Robux (VNĐ cho 1 Robux)</label>
            <input 
              type="number" 
              value={robuxRate} 
              onChange={(e) => setRobuxRate(e.target.value)}
              style={{ width: '100%', padding: '10px', backgroundColor: '#1a1a24', border: '1px solid rgba(255,255,255,0.1)', color: '#fff', borderRadius: '8px' }}
            />
          </div>
          ...
          <div className={styles.formGroup} style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
            ...
            />
            <label htmlFor="maintenance">Chế độ bảo trì (Tắt toàn bộ tính năng mua hàng)</label>
          </div>

          <div style={{ marginTop: '1rem', borderTop: '1px solid rgba(255,255,255,0.1)', paddingTop: '1.5rem' }}>
            <button className="btn btn-primary" style={{ padding: '0.8rem 2rem' }} onClick={handleSave}>
              Lưu cấu hình
            </button>
          </div>
        </div>
